...
def analyze_pcap():
    ...
    try:
        ...
        # Get quick file size estimate for packet count
        estimated_packets = 0
        try:
            file_size = os.path.getsize(pcap_file)
            # Rough estimate based on typical PCAP packet size of ~500 bytes per packet
            estimated_packets = int(file_size / 500)
        except Exception as e:
            print(f"Error estimating packet count: {e}")
            estimated_packets = 100000  # Default fallback
        ...
        return jsonify({
            'status': 'success', 
            'message': f'Started analyzing {pcap_file}',
...
def analyze_pcap_file(pcap_file):
    ...
    try:
        ...
        # Send immediate notification that analysis has started
        socketio.emit('pcap_progress', {
            'progress': 1,  # Start at 1% instead of 0%
            'processed_packets': 0,
            'file': current_pcap_file
        })
        
        # Better packet estimation based on file size and sampling
        estimated_total = 100000  # Default estimate
        analysis_start_time = time.time()
        
        try:
            print("Estimating total packet count...")
            
            # Get file size for a quick initial estimate
            file_size = os.path.getsize(pcap_file)
            
            # Faster estimate based on file size (assuming average packet size of ~500 bytes)
            quick_estimate = int(file_size / 500)
            
            # Sample a small portion to refine the estimate
            sample_size = min(5000, int(quick_estimate * 0.05))  # Sample up to 5000 packets or 5% of estimate
            sampled_bytes = 0
            ...
            with PcapReader(pcap_file) as pcap_reader:
                for _ in range(sample_size):
                    try:
                        ...
                        packet_count += 1
                        if hasattr(packet, 'original_len'):
                            sampled_bytes += packet.original_len
                        else:
                            # Fallback if original_len not available
                            sampled_bytes += len(bytes(packet))
                    except StopIteration:
                        break
            
            # If we got enough samples, refine the estimate
            if packet_count > 0 and sampled_bytes > 0:
                avg_packet_size = sampled_bytes / packet_count
                estimated_total = int(file_size / avg_packet_size)
                print(f"Estimated ~{estimated_total} packets based on avg size of {avg_packet_size:.2f} bytes")
            else:
                # Fallback to quick estimate
                estimated_total = quick_estimate
                print(f"Using file size estimate: ~{estimated_total} packets")
            
            # Cap at a reasonable value for UI
            estimated_total = min(estimated_total, 2000000)
            
            # Update progress with improved estimate
            socketio.emit('pcap_progress', {
                'progress': 2,  # Move to 2%
                'processed_packets': 0,
                'file': current_pcap_file,
                'estimated_packets': estimated_total
            })
            
            # Process packets
            processed = 0
            batch_size = 100  # Process in batches of 100 for smoother updates
            packet_batch = []
            ...
            with PcapReader(pcap_file) as pcap_reader:
                for packet in pcap_reader:
                    ...
                    if len(packet_batch) >= batch_size:
                        ...
                        if processed < estimated_total:
                            pcap_analysis_progress = min(int((processed / estimated_total) * 100), 95)
                        else:
                            # If we've exceeded the estimate, cap at 95%
                            pcap_analysis_progress = 95
                        ...
                        time.sleep(0.005)  # Reduce delay for faster processing
                    
                    # For extremely large files, process a reasonable subset
                    if processed >= 1000000:  # Increased to 1M packets
                ...
                duration_str = f"{analysis_duration:.2f} seconds"
                ...
                print(f"Completed analysis of {pcap_file} in {duration_str}")
                print(f"Total alerts generated: {len(alerts)}")
                
                # Set final progress to 100%
                pcap_analysis_progress = 100
                
                # Get alert distribution for summary
                alert_counts = {
                    'port_scan': alert_distribution.get('Port Scan', 0),
                    'dos': alert_distribution.get('DoS Attack', 0),
                    'ddos': alert_distribution.get('DDoS Attack', 0),
